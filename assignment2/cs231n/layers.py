...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_hat, batch_var, gamma, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == "test":
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x_hat, batch_var, gamma, eps = cache
    N = x_hat.shape[0]
    ...
    dx = (
        1
        / np.sqrt(batch_var + eps)
        * (
            dx_hat
            - 1 / N * np.sum(dx_hat, axis=0)
            - 1 / N * x_hat * np.sum(dx_hat * x_hat, axis=0)
        )
    )
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out = gamma * x_hat + beta
    cache = (x_hat, gamma, x_mean, x_var, eps)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    ###########################################################################
    x_grouped = x.reshape(x.shape[0], G, -1)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dx_hat_grouped = dx_hat.reshape(N, G, -1)
    x_hat_grouped = x_hat.reshape(N, G, -1)
    D = dx_hat_grouped.shape[2]
    dx_grouped = (
        1
        / np.sqrt(var + eps)
        * (
            dx_hat_grouped
            - 1 / D * np.sum(dx_hat_grouped, axis=2, keepdims=True)
            - 1 / D * x_hat_grouped * np.sum(dx_hat_grouped * x_hat_grouped, axis=2, keepdims=True)
        )
    )
